...
2. Sends each new transcript to the Hermes webhook for AI analysis + Telegram delivery.
3. Extracts structured tasks via Gemini and creates them in Google Tasks.
4. Records processed file IDs in a GCS state bucket to avoid duplicates.
...
Required environment variables (set in Cloud Function config):
  ...
  HERMES_WEBHOOK_SECRET — HMAC secret configured in Hermes for this subscription
  GCS_STATE_BUCKET      — GCS bucket name used to store processed file IDs
  GEMINI_API_KEY        — Google AI Studio API key (for task extraction)
  GOOGLE_TASKS_LIST_ID  — Google Tasks list ID to add tasks to (use @default for default list)
  TASKS_DELIVERY_URL    — (optional) Your dashboard /api/ingest/tasks URL for task storage
  TASKS_DELIVERY_TOKEN  — (optional) Dashboard DASHBOARD_TOKEN value
"""
